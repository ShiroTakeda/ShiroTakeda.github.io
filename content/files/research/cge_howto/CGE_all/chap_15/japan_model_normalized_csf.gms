$title 日本の CGE モデル
$ontext
Time-stamp: <2022-01-23 18:58:08 st>
First-written:  <2016/03/15>

+ このプログラムは Japan_model.gms と基本的に同じ
+ ただし
  + Calibrated share form を用いて記述
  + 内生変数の値の初期値を 1 に規準化
  + ベンチマーク均衡において 1 をとる変数を表すパラメータ（例えば、
    p0(j), p_a0(i), p_f0(f), c_a0(i) 等）は実質的には意味を持たないの
    で、全て省略している。
...
$if not setglobal sim_name $setglobal sim_name normalized_csf
...
*   --------------------------------------------------------------
*   Calibrated share form 用のシェアの値
$ontext
$offtext

*   基準均衡において税がかかっている価格の Agent 価格を表すパラメー
*   タ。表記を簡潔にするために利用。
parameter
    ap_pf0      生産要素の Agent 価格
    ap_pm0      輸入財の Agent 価格
    ap_pc0      消費財の Agent 価格
;
ap_pf0(f,j) = (1+rt_f0(f,j))*p_f0(f);
ap_pm0(i) = (1+rt_m0(i))*p_m0(i);
ap_pc0(i) = (1+rt_c0(i))*p_a0(i);
display ap_pf0, ap_pm0, ap_pc0;

parameter
    sh_y(i,j)   生産額における各財のシェア
    sh_x(i,j)   生産費用に占める中間投入物のシェア
    sh_va(j)    生産費用に占める合成生産要素のシェア
    sh_f(f,j)   各生産要素のシェア
    sh_ad(i)    Armington 統合における国内財のシェア
    sh_am(i)    Armington 統合における輸入財のシェア
    sh_es(i)    輸出向け・国内向け配分における輸出向けのシェア
    sh_ds(i)    輸出向け・国内向け配分における国内向けのシェア
    sh_d(i)     消費における各財のシェア
    sh_inv(i)   投資財生産における各財のシェア
    sh_gov(i)   政府消費財生産における各財のシェア
;
*   生産額における各財のシェア
sh_y(i,j)$a_y0(i,j)
    = p_y0(i)*a_y0(i,j) / sum(ii, p_y0(ii)*a_y0(ii,j));

*   生産費用に占める中間投入物のシェア
sh_x(i,j)$a_x0(i,j)
    = p_a0(i)*a_x0(i,j) / (sum(ii, p_a0(ii)*a_x0(ii,j)) + p_va0(j)*a_v0(j));

*   生産費用に占める中間投入物のシェア
sh_va(j)$a_v0(j)
    = p_va0(j)*a_v0(j) / (sum(ii, p_a0(ii)*a_x0(ii,j)) + p_va0(j)*a_v0(j));

*   各生産要素のシェア
sh_f(f,j)$a_f0(f,j)
    = ap_pf0(f,j)*a_f0(f,j) / sum(ff, ap_pf0(ff,j)*a_f0(ff,j));

*   Armington 統合における国内財のシェア
sh_ad(i)$a_ad0(i)
    = p_d0(i)*a_ad0(i) / (p_d0(i)*a_ad0(i) + ap_pm0(i)*a_am0(i));

*   Armington 統合における輸入財のシェア
sh_am(i)$a_am0(i)
    = ap_pm0(i)*a_am0(i) / (p_d0(i)*a_ad0(i) + ap_pm0(i)*a_am0(i));

*   輸出向け・国内向け配分における輸出向けのシェア
sh_es(i)$a_es0(i)
    = p_e0(i)*a_es0(i) / (p_e0(i)*a_es0(i) + p_d0(i)*a_ds0(i));

*   輸出向け・国内向け配分における国内向けのシェア
sh_ds(i)$a_ds0(i)
    = p_d0(i)*a_ds0(i) / (p_e0(i)*a_es0(i) + p_d0(i)*a_ds0(i));

*   消費における各財のシェア
sh_d(i)$a_d0(i)
    = ap_pc0(i)*a_d0(i) / sum(ii, ap_pc0(ii)*a_d0(ii));

*   投資財生産における各財のシェア
sh_inv(i)$a_inv0(i)
    = p_a0(i)*a_inv0(i) / sum(ii, p_a0(ii)*a_inv0(ii));

*   政府消費財生産における各財のシェア
sh_gov(i)$a_gov0(i)
    = p_a0(i)*a_gov0(i) / sum(ii, p_a0(ii)*a_gov0(ii));

display sh_y, sh_x, sh_va, sh_f, sh_ad, sh_am, sh_es, sh_ds, sh_d,
        sh_inv, sh_gov;
...
e_c(j)$y0(j) ..
    c(j) =e=
    sum(i$sh_x(i,j), sh_x(i,j)*p_a(i)) + sh_va(j)*p_va(j);

*   合成生産要素生産の単位費用
e_c_va(j) ..
    c_va(j) =e=
    (sum(f$sh_f(f,j),
        sh_f(f,j)*((1+rt_f(f,j))*p_f(f)/ap_pf0(f,j))**(1-sig_v(j)))
    )**(1/(1-sig_v(j)));
...
e_c_u ..
    c_u =e=
    (sum(i$sh_d(i),
        sh_d(i) * ((1+rt_c(i))*p_a(i)/ap_pc0(i))**(1-sig_c))
    )**(1/(1-sig_c));

*   Armington統合の単位費用
e_c_a(i)$q_a0(i) ..
    c_a(i) =e=
    ((sh_ad(i) * (p_d(i)/p_d0(i))**(1-sig_dm(i)))$sh_ad(i)
    + (sh_am(i) * ((1+rt_m(i))*p_m(i)/ap_pm0(i))**(1-sig_dm(i)))$sh_am(i)
    )**(1/(1-sig_dm(i)));

*   投資財生産の単位費用
e_c_inv ..
    c_inv =e= sum(i$sh_inv(i), sh_inv(i)*p_a(i));

*   政府消費財生産の単位費用
e_c_gov ..
    c_gov =e= sum(i$sh_gov(i), sh_gov(i)*p_a(i));

*   生産の単位収入
e_r_y(j)$y0(j) ..
    r_y(j) =e= sum(i$sh_y(i,j), sh_y(i,j)*p_y(i));

*   輸出・国内供給配分活動の単位収入
e_r_de(i) ..
    r_de(i) =e=
    ((sh_es(i) * (p_e(i))**(1+eta_de(i)))$sh_es(i)
        + (sh_ds(i) * (p_d(i))**(1+eta_de(i)))$sh_ds(i)
    )**(1/(1+eta_de(i)));
...
*   生産水準
e_y(j)$y0(j) .. c0(j)*c(j) =e= (1 - rt_y(j)) * r_y(j);
...
*   単位生産要素需要
$macro a_f(f,j)  \
    (a_f0(f,j) * (c_va(j) / (((1+rt_f(f,j))*p_f(f))/ap_pf0(f,j)))**(sig_v(j)))

*   単位消費需要
$macro a_d(i)  (a_d0(i) * (c_u / (((1+rt_c(i))*p_a(i))/ap_pc0(i)))**(sig_c))

*   Armington統合の単位国内財需要
$macro a_ad(i)  (a_ad0(i) * (c_a(i) / p_d(i))**(sig_dm(i)))

*   Armington統合の単位輸入財需要
$macro a_am(i)  (a_am0(i) * (c_a(i) / (((1+rt_m(i))*p_m(i))/ap_pm0(i)))**(sig_dm(i)))

*   単位輸出供給
$macro a_es(i)  (a_es0(i) * (p_e(i) / (r_de(i)/r_de0(i)))**(eta_de(i)))

*   単位国内供給
$macro a_ds(i)  (a_ds0(i) * (p_d(i) / (r_de(i)/r_de0(i)))**(eta_de(i)))
...
*   Armington財の市場均衡
e_p_a(i) .. q_a0(i)*q_a(i) =e= sum(j$a_x0(i,j), a_x0(i,j)*y0(j)*y(j))
        + (a_d(i)*u0*u)$a_d0(i)
...
*   効用の市場均衡
e_p_u ..    p_u * u0*u =e= m_d;

*   投資財の市場均衡
e_p_inv .. q_inv0*q_inv - q_inv_bar =e= 0;

*   政府消費財の市場均衡
e_p_gov .. q_gov0*q_gov =e= m_gov / p_gov;

*   貿易収支の定義
e_ts .. ts =e= sum(i$x_e0(i), p_ew0(i)*x_e0(i)*x_e(i)) - sum(i$x_m0(i), p_mw0(i)*x_m0(i)*x_m(i));
...
*   税収（一括税以外）
e_tax_all .. tax_all =e=
         sum((f,j)$a_f0(f,j), rt_f(f,j)*p_f(f)*a_f(f,j)*v_a0(j)*v_a(j))
         + sum(i$x_m0(i), rt_m(i)*p_m(i)*x_m0(i)*x_m(i))
         + sum(i$a_d0(i), rt_c(i)*p_a(i)*a_d(i)*u0*u)
         + sum(j$y0(j), rt_y(j)*r_y(j)*y0(j)*y(j))
...
*   消費に支出される所得
e_m_d .. m_d =e= m - p_inv*q_inv0*q_inv - p_ex*ts;
...
*     chk_c;
* chk_c(j,"lhs") = c.l(j);
* chk_c(j,"rhs") = c0(j)*(sum(i$sh_x(i,j), sh_x(i,j)*(p_a.l(i)/p_a0(i))) + sh_va(j)*(p_va.l(j))/p_va0(j));
* display chk_c, c0;
