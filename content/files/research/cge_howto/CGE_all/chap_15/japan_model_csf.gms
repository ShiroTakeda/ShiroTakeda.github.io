$title 日本の CGE モデル
$ontext
Time-stamp: <2022-01-23 18:20:34 st>
First-written:  <2016/03/15>

+ このプログラムは Japan_model.gms と基本的に同じモデル。
+ ただし、Calibrated share form を用いて記述している。
...
$if not setglobal sim_name $setglobal sim_name csf
...
e_c(j)$y0(j) ..
    c(j) =e=
    c0(j)*(sum(i$sh_x(i,j), sh_x(i,j)*(p_a(i)/p_a0(i))) + sh_va(j)*(p_va(j))/p_va0(j));
...
*   投資財生産の単位費用
e_c_inv ..
    c_inv =e= c_inv0 * sum(i$sh_inv(i), sh_inv(i)*(p_a(i)/p_a0(i)));

*   政府消費財生産の単位費用
e_c_gov ..
    c_gov =e= c_gov0 * sum(i$sh_gov(i), sh_gov(i)*(p_a(i)/p_a0(i)));

*   生産の単位収入
e_r_y(j)$y0(j) ..
    r_y(j) =e= r_y0(j) * sum(i$sh_y(i,j), sh_y(i,j)*(p_y(i)/p_y0(i)));

*   輸出・国内供給配分活動の単位収入
e_r_de(i) ..
    r_de(i) =e=
    r_de0(i)
    * ((sh_es(i) * (p_e(i)/p_e0(i))**(1+eta_de(i)))$sh_es(i)
    + (sh_ds(i) * (p_d(i)/p_d0(i))**(1+eta_de(i)))$sh_ds(i)
    )**(1/(1+eta_de(i)));
...
*   単位生産要素需要
$macro a_f(f,j)  \
    (a_f0(f,j) * ((c_va(j)/c_va0(j)) / (((1+rt_f(f,j))*p_f(f))/ap_pf0(f,j)))**(sig_v(j)))

*   単位消費需要
$macro a_d(i)  (a_d0(i) * ((c_u/c_u0) / (((1+rt_c(i))*p_a(i))/ap_pc0(i)))**(sig_c))

*   Armington統合の単位国内財需要
$macro a_ad(i)  (a_ad0(i) * ((c_a(i)/c_a0(i)) / (p_d(i)/p_d0(i)))**(sig_dm(i)))

*   Armington統合の単位輸入財需要
$macro a_am(i)  (a_am0(i) * ((c_a(i)/c_a0(i)) / (((1+rt_m(i))*p_m(i))/ap_pm0(i)))**(sig_dm(i)))

*   単位輸出供給
$macro a_es(i)  (a_es0(i) * ((p_e(i)/p_e0(i)) / (r_de(i)/r_de0(i)))**(eta_de(i)))

*   単位国内供給
$macro a_ds(i)  (a_ds0(i) * ((p_d(i)/p_d0(i)) / (r_de(i)/r_de0(i)))**(eta_de(i)))
