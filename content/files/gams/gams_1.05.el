;;; GAMS.EL --- Major mode for editing and viewing GAMS program files.
...
;; Time-stamp: <2001-09-04 19:53:58 ged9203>
;; Version: 1.05
...
;;; Commentary:
;;
;; This is a major mode for editing GAMS program files in Emacs.
;;
;; This program include two major modes (GAMS mode and GAMS-LST mode).
;; GAMS mode is for editing GMS files and GAMS-LST mode for viewing GAMS
;; LST files.
;;
;; The latest version of this program may be available at
...
;;
;;
;; There is a sample file for settings (`emacs-sample').  Please see it,
;; too.
;;
;; There are several lisp variables which decide important behavior of
;; GAMS and GAMS mode.  So, Please read the customization part below, too.
;;
...
;;	`gams-insert-comment'.  % indicates the cursor place and will disappear
...
;;
;;  * Add a variable `gams-file-extension'.  This variable is a list of
;;    extensions of files in which GAMS mode starts automatically.
;;  * Changed the setting in .emacs.  Now you don't need to set
;;    auto-mode-alist and autoload in .emacs.  Just write (load "gams").
;;    See the file `emacs-sample'.
...
;;
;;  * Added a new function `gams-lst-jump-to-error-file' binded to `g'
;;    which was previously binded to `gams-lst-view-gms'.  Now you can
;;    jump to error places in included files inserted by, for example,
;;    $libinclude or $batiinclude statements.
;;  * Add font-lock (colorization) to GAMS-LST mode so that keyword VAR,
...
;;  * Define original faces for GAMS mode and GAMS-LST mode.
...
(defconst gams-el-version-number "1.05"
  "Version of gams.el")
...
(defvar gams-file-extension '("gms")
...
(defun gams-list-upcase (list-name)
  (let* (list-new ele)
    (while list-name
      (setq ele (upcase (car list-name)))
...
(setq auto-mode-alist
	(cons
	 (cons
	  (format "\\.\\(xyz%s\\)$" (gams-create-regexp
...
If FLAG is t, it means that use upper case."
    (let ((list-name list)
	  gams-alist)
      (if (null flag)
...
(setq gams-lst-font-lock-keywords
  (list
...
;  (define-key map "g" 'gams-lst-view-gms)
  (define-key map "g" 'gams-lst-jump-to-error-file)
...
'\\[gams-lst-jump-to-error-file]' - Jump back to the error place in program files.
	...
	(progn
	  ...
	  (setq error-num
		(buffer-substring
		 (match-beginning 2)
		 (match-end 2)))
	  (message "If you want to jump to the error place in the program file, push `g'.")
	  (if error-num
	      (progn
...
(defun gams-lst-jump-to-error-file ()
  (interactive)
  (let (point-a point-b line-num file-name error-column)

    (gams-lst-jump-to-error)

    ; Store column number.
    (setq error-column (- (current-column) 7))
	...
	(progn
	  (setq line-num (buffer-substring
			  (match-beginning 1)
			  (match-end 1)))
      ...
      (setq file-name (gams-lst-get-gms-filename))
...
; (defun gams-lst-jump-to-error-file ()
;   (interactive)
;   (let (point-a point-b line-num file-name)
...
;       (next-line -2)
;       (beginning-of-line)
;       (re-search-forward "^[ ]*\\([0-9]+\\)[ ]+" point-a t)
;       (setq line-num (buffer-substring
; 		      (match-beginning 1)
; 		      (match-end 1)))
;       (setq file-name (gams-lst-get-gms-filename))
