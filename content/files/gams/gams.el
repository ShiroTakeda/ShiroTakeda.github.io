...
;; Copyright (C) 2001, 2002, 2003, 2004 Shiro Takeda
;; Version: 2.1.2
;; $Id: gams.el,v 1.20 2005/09/02 07:39:53 st Exp $
;; Time-stamp: <2005-09-02 13:23:29 Shiro Takeda>
...
(defconst gams-mode-version "2.1.2"
...
For example, \"c:/Program Files/Adobe/Acrobat 5.0/Reader/AcroRd32.exe\"."
  :type 'file
  :group 'gams)
...
(defcustom gams-ol-item-name-width 18
...
(defvar gams-manuals-alist
	...
	("Solver-XPRESSLICENSING" . "xpresslicensing"))
      "Alist of the name of GAMS manuals and its abbreviated name (label).
...
(defvar gams-meadow2 (and (featurep 'meadow) (= emacs-major-version 21)))
...
(defvar gams:shell-c 
  (or (and (boundp 'shell-command-option) shell-command-option)
      ...
      (if (string-match "command.com\\|cmd.exe\\|start.exe" shell-file-name)
	  "/c" "-c"))
...
;;   '((((class color) (background light)) (:foreground "#f00040" :bold nil))
;;     (((class color) (background dark)) (:foreground "red" :bold nil)))
;;   "Face for misc."
;;   :group 'gams-faces)
...
(defface gams-highline-face
  '((((class color) (background light))
     (:bold nil :foreground "#f0f0f0" :background "#009000"))
    (((class color) (background dark))
...
(defface gams-highline-sub-face
  '((((class color) (background light))
     (:foreground "#f0f0f0" :background "purple"))
    (((class color) (background dark))
...
(defsubst gams-store-point-sol-sum (limit)
  ...
  (let (beg end)
    (when (re-search-forward "SUM[ \t]+\\(SOLVE SUMMARY[ \t]+SOLVER[ \t]+STATUS[ \t]+=\\( [^1]\\| 1, MODEL STATUS = [^128]\\)\\)" limit t)
...
(defsubst gams-font-lock-match-regexp (keywords limit beg end)
...
(defun gams-store-point-special-comment (limit)
  ...
  (let ((key
	 (concat "\\(----[ ]+[0-9]+[ ]+"
		 (gams-regexp-opt (list gams-special-comment-symbol))
		 "[^\n]*\\)")))
...
;; (setq gams-font-lock-keywords-1
	...
	;; texts in slash pair.
;;	(gams-store-point-slash (0 gams-slash-face append t t))
...
(defun gams-store-point-slash (limit)
  ...
  (let (cur-po beg end flag beg-decl po-a)
	...
	(when (and gams-xemacs (not (equal 0 (current-column)))) ; koko dame.
	  (forward-line 1))
;;	(if (and t (re-search-forward "/" nil t))
...
;;			  (search-forward "/" nil t)
		    ...
		    ;; Inside slash pair.
		    (cond
;;		     ((not (save-excursion (re-search-forward "/" nil t)))
...
(defun gams-store-point-explanation (limit)
  ...
  (let ((cur-po (point))
	decl-end
	flag cont beg end ontext po-a po-b)
    ;; For XEmacs.
    ;;     (when (not (equal 0 (current-column)))
    ;;       (forward-line 1))
    (catch 'found
      (while t
	...
	(cond
	 ...
	 ((and (not ontext)
;; 	       (setq po-b (gams-in-declaration-p t)))
 	  ...
 	  (if (not (< (point) limit))
		...
		(forward-char 1)
	      ...
	      (setq cont
		    (gams-store-point-explanation-get-explanation
		     po-b cur-po decl-end limit))
	      (when cont
...
CURRENT is the current point.  END is the point of the declaration block."
  (let ((lim (min limit end))
	(eol-sym (concat "[" gams-eolcom-symbol "]"))
	(inl-sym (concat "[" gams-inlinecom-symbol-start "]"))
...
(defun gams-store-point-inlinecom (limit)
  ...
  (let (beg end flag)
    (when gams-inlinecom-symbol-start
      (catch 'found
	(while t
	  (if (not (search-forward gams-inlinecom-symbol-start limit t))
	      (throw 'found t)
	    (setq beg (match-beginning 0))
	    (when (not (gams-in-quote-p-extended))
	      (when (search-forward gams-inlinecom-symbol-end limit t)
...
(defun gams-store-point-eolcom (limit)
  ...
  (let (flag beg)
    (when gams-eolcom-symbol
      (catch 'found
	(while t
	  (if (not (search-forward gams-eolcom-symbol limit t))
	      (throw 'found t)
	    (setq beg (match-beginning 0))
	    (when (not (gams-in-quote-p-extended))
	      (end-of-line)
...
(defun gams-mode-key-update ()
  (let ((map gams-mode-map))
      ...
      (define-key map "\C-c.." 'gams-show-identifier)
...
\\[gams-popup-process-bu2ffer]   	Popup GAMS process buffer.
...
(define-derived-mode gams-ps-mode fundamental-mode "GAMS-PS"
  ...
  (make-local-variable 'gams-ps-compile-start-time)
...
Envoke the pdf file viewer and see GAMS manuals.  The pdf file viewer is
determined by the variable `gams-docs-view-program'.  The directory of
GAMS documents is determined by the variable `gams-system-directory'.
This command does not work well for GAMS21.2. because of the change in
directory structures of GAMS distribution."
  (interactive)
  (unwind-protect
      (let* ((gams-alist gams-manuals-alist)
	     (completion-ignore-case t)
	     (docs-dir
	      (concat (file-name-directory gams-system-directory) "docs"))
	     key1
	     (source-window (selected-window))
	     guess
	     (statement
		...
		(gams-read-docs
	     ...
	     ;; directory name.
	     (dir-name
	      (if (string-match "/" docs-dir (string-width docs-dir))
		  (substring docs-dir 1 (- (string-width docs-dir)))
		docs-dir))
	     file-name
	     dir-docs
...
(defun gams-sid-goto-inline-comment-end ()
  (let ((end (line-end-position)))
    (forward-char 1)
    (or (search-forward gams-inlinecom-symbol-end end t) end)))
...
(defun gams-sid-show-result-def (name po-def len po-beg fname)
  (let ((cur-po (point)))
    (current-buffer)
    (if po-def
	(cond
...
(defun gams-sid-show-result-mess (name fname)
  (let* ((file (if gams-sid-in-subroutine-p
		   fname
...
(defun gams-sid-get-alist (name)
  ...
  (let ((id-name (downcase name))
	...
	(cond
	 ...
	 ((looking-at "[ \t]")
	  (skip-chars-forward "[ \t]"))
	 ((looking-at (concat "[" gams-eolcom-symbol "]"))
	  (forward-line 1))
	 ((looking-at (concat "[" gams-inlinecom-symbol-start "]"))
	  (gams-sid-goto-inline-comment-end))
	 ((looking-at "\n")
	  (when f-id
	    (setq f-id nil))
...
(defun gams-sil-make-marker (alist)
  (let ((temp alist))
    (mapcar
     (lambda (x)
       (list (nth 0 x)
	     ...
	     (nth 2 x)
	     (nth 3 x)
	     (set-marker (make-marker) (nth 1 x))))
     alist)))
...
This command cannot identify aliased set identifer."
  ...
  (let* ((curr-buff (current-buffer))
    ...
    (unless already-p
	  ...
	  nil
	  (condition-case err
	      (setq gams-identifier-symbol-temp
...
(defun gams-sil-rescan ()
  (interactive)
  (let ((cur-line (count-lines (point-min) (+ 1 (point))))
	(cur-buff (current-buffer))
	ident-alist)
...
(defvar gams-sil-font-lock-keywords
  '(("^[0-9]+[ \t]+\\(PAR\\)" (1 gams-lst-par-face))
    ("^[0-9]+[ \t]+\\(SET\\)" (1 gams-lst-set-face))
    ("^[0-9]+[ \t]+\\(VAR\\)" (1 gams-lst-var-face))
    ("^[0-9]+[ \t]+\\(EQU\\)" (1 gams-lst-equ-face))
    ("^[0-9]+[ \t]+\\(MOD\\)" (1 gams-ol-loo-face))
    ("^[0-9]+[ \t]+\\(MPS\\)" (1 gams-sil-mpsge-face))
    ))
...
.       In other window, show the original position.
?	Show this help.")
    (goto-char (point-min))
    (setq buffer-read-only t)
    (select-window (next-window nil 1))
...
------------------------------------------------------------------------------\n"
    ...
    (while a-list
      ...
      (indent-to gams-sil-type-position-beg)
      (insert (nth 0 ele))
      (indent-to 15)
      (insert (or (nth 2 ele) ""))
...
(setq gams-regexp-declaration-4-temp
      "\\(^$[s]?title[ \t]+\\)\\|^[ \t]*\\(solve[ \t]+\\)[a-zA-Z_]+\\|^[ \t]*\\(parameter[s]?\\|set[s]?\\|scalar[s]?\\|table\\|alias\\|acronym[s]?\\|\\(free\\|positive\\|negative\\|binary\\|integer\\)*[ \t]*variable[s]?\\|equation[s]?\\|model[s]?\\|$model:\\)[ \t\n(]*")
	  ...
	  (while t
		...
		(progn
		  ...
		  (message "Starting GAMS-SIL mode.%s"
			   (make-string (min (/ count 50) (- fill-column 20)) ?.))
		  (cond
		   ((match-beginning 1)
		    (goto-char (match-end 1))
		   ...
		   (t
		    ...
		    (setq match-decl (gams*buffer-substring (match-beginning 4)
							    (match-end 4)))
		    (cond
...
(defun gams-sil-get-alist (type)
  (let (alist po-beg ex-beg ex-end po id exp f-id)
    (catch 'found
      (while t
	;; Skip irrelevant lines.
	(while (gams-check-line-type)
...
(define-key gams-mode-map "\C-c\C-n" 'gams-insert-statement-extended)
...
(defun gams-insert-post-solve (name)
  (let ((def-solv (or gams-insert-solver-type-previous
    ...
    (if (equal sol-type "")
	(progn (setq sol-type def-solv) (insert (concat sol-type " ")))
      (setq gams-insert-solver-type-previous sol-type)
      (insert (concat sol-type " ")))
...
Completion of internal file name."
  ...
  (unwind-protect
	...
	(insert statement)
	(let ((func-name (cdr (assoc statement gams-statement-alist-ext))))
	  (when func-name
...
and show its meaning in another window if error number is displayed."
  (interactive)
  (goto-char (point-min))
  (let (error-num error-place error-mes-place error-column b-point a-point)
	...
	(progn
	  ...
	  (progn
		...
		(message (format "Error is found!  Type `%s' if you want to jump to the error line %s."
...
(defun gams-lst-jump-to-error-file ()
  ...
  (let (point-b line-num file-name error-column temp-col
	...
	(progn
	  ...
	  (forward-line 1)
	  (save-excursion (forward-line 1) (setq point-b (point)))
...
If BUFFER is non-nil, the current buffer is OL buffer, not LST buffer."
  (let ((cur-buf (if buffer (current-buffer) nil))
	...
	(progn
	  ...
	  (current-buffer)
	  (setq alist gams-ol-alist-tempo)
	  (switch-to-buffer cur-buf))
	;; If LST buffer, return the value of `gams-ol-alist'.
      (setq alist gams-ol-alist))
...
(defun gams-ol-view-base ()
  ...
  (let* ((line-num (gams-ol-count-line))
	 (list-par (assoc line-num (gams-ol-get-alist t)))
	 (point-par (car (cdr list-par)))
	 (flag gams-ol-mark-flag))
    (when list-par (gams-ol-view-base-internal point-par flag))
    (gams-ol-show-key)))
 
(defun gams-ol-view-base-internal (point &optional flag)
  (let 	((cur-buf (current-buffer))
	...
	(if (not flag)
	    (progn
	      (split-window
	       ...
	       (+ 1 gams-ol-height))
	      (other-window 1)
	      (set-window-buffer (selected-window) gams-ol-lstbuf)
	  ...
	  (split-window
	   ...
	   (/ (window-height) 2))
	  (if gams-ol-use-external
	      (goto-line (car (cdr (assoc flag (gams-ol-get-alist)))))
...
;;(defun gams-ol-report-sum ()
  ...
  (let (temp-po cont var-1 var-2 var-3 var-4 var-5)
    (save-excursion
      ...
      (if (re-search-forward "\\([0-9]+\\)[ \t]+REDEFINED" nil t)
	  (setq var-4 (gams*buffer-substring (match-beginning 1)
					     (match-end 1)))
	(forward-line -1))
      (forward-line 1)
      (setq temp-po (line-end-position))
      (if (re-search-forward "\\([0-9]+\\)[ \t]+ERRORS" nil t)
    ...
    (setq cont
	  (concat "[" var-1 ", "
		  var-2 ", "
		  var-3
		  (if var-4 (concat ", " var-4) nil)
		  (if var-5 (concat ", " var-5) nil)
		  "]"
		  )))
    cont))
	  ...
	  (let (po-so)
	    ...
	    (setq malist
		  (cons (list count pobeg "SUM" "SOLVE SUMMARY"
			...
			malist))
	    (end-of-line))))))
    (setq gams-ol-alist (reverse malist))))
...
`gams-outline-external', too."
  (let* ((cur-buf (buffer-name))
    ...
    (switch-to-buffer cur-buf)
    (kill-buffer out-buf)
    (setq gams-ol-alist gams-ol-alist-temp-alist)
    ))

(defun gams-ol-convert (alist)
  "Convert `gams-ol-alist' to the alist with only viewable items."
  (interactive)
  (let ((list-count 1) temp-element)
    (if (rassoc nil gams-ol-view-item)
	(setq gams-ol-alist-tempo
	      (delete nil
		      (mapcar '(lambda (x)
				 (setq temp-element (car (cdr (cdr x))))
				 (when (cdr (assoc temp-element gams-ol-view-item))
				   (setcar x list-count)
...
Otherwise nil."
  (let* ((cur-po (point))
	 ...
	 (inl-end gams-inlinecom-symbol-end)
	 (beg (line-beginning-position))
	 (end (line-end-position))
	 cont flag reg)
    (save-excursion
	...
	(setq reg (concat "[" eol "]" "\\|" "[" inl "]")))
       (eol (setq reg (concat "[" eol "]")))
       (inl (setq reg (concat "[" inl "]"))))
	...
	(while t
	  (if (and reg (re-search-backward reg beg t))
		...
		(if (equal cont inl)
		    (when (and (re-search-forward (concat "[" inl-end "]") end t)
			       (<= cur-po (point)))
		      (setq flag t))
		  (setq flag t))
		(throw 'found t))
...
"
  (let* ((line-1 1) (line-2 1)
    ...
    (save-excursion
      (forward-line -1)
      (while (>= (point) beg)
	(if (not (gams-check-line-type flag-a))
...
(defun gams-calculate-indent-decl (beg)
  ...
  (let* ((temp (gams-judge-line beg))
	...
	(cond
	 ((equal line 2)
	  (if (equal "s" (gams-check-line-type nil nil t))
	      (progn
	...
	(cond
	 ...
	 (t
	  ...
	  (cond
	   ...
	   ;; If the previous line includes no slash.
	   (t
	    (if (not (equal "s" (gams-check-line-type nil nil t)))
		;; If current line does not starts with slash.
		(setq indent-num (gams-calculate-indent-previous pre-line))
	      ;; If the current line starts with slash.
	      (if (not (gams-slash-end-p beg))
...
(defun gams-calculate-indent-other (beg &optional new)
  ...
  (let* ((temp (gams-judge-line beg))
	 (line (car (cdr temp)))
	 (back (car (cdr (cdr temp)))))
    (save-excursion
      (if (equal line 2)
	  (if new gams-indent-number nil)
	(if new (gams-calculate-indent-previous back) nil)))))
...
Otherwise nil."
  (let ((cur-po (point))
	...
	(when (equal flag-beg "on")
	  ...
	  (when (re-search-forward "^[ \t]*$model" cur-po t)
	    (setq flag t)))))
    flag))

;;; New functions.
(defun gams-in-loop-block-p (beg)
  "Calculate the number of loop before the current point.
...
(defun gams-calculate-indent-loop (begin new end)
  (let* ((cur-po (point))
	 (temp-list (gams-in-loop-block-p begin))
	 (list-loop temp-list)
	 (count (length temp-list))
	 (end-po begin) po-a indent)
    ;; Calculate the number of loop.
    ;; Retrun the loop level.
    (save-excursion
      (while temp-list
	(when (setq po-a (gams-loop-end-p (car temp-list) cur-po))
	  (when (> po-a end-po)
	    (setq end-po po-a))
	  (setq count (- count 1)))
	...
	(when (not end)
	  (catch 'found
	    (while t
		  ...
		  (throw 'found t)
		(goto-char (match-end 1))
		(when (and (equal (* count gams-indent-number-loop)
				  (current-column))
			   (not (looking-at "else")))
		  (beginning-of-line)		  
		  (setq po-a (point))
		  (if (looking-at (concat "^[ \t]*" gams-regexp-put))
		      ;; Put block.
		      (progn
			(goto-char cur-po)
			(when (not (gams-block-end-p po-a new))
			  (setq indent (gams-calculate-indent-put po-a new))))
		    ;; Other block.
		    (goto-char cur-po)
		    (when (not (gams-block-end-p po-a new))
		      (setq indent (+ gams-indent-number (* count gams-indent-number-loop)))
		      ))
		  (throw 'found t))))))))
    (if (not indent)
	(setq indent (* gams-indent-number-loop count)))
    indent))
  ...
  (let ((new (if gams-indent-equation-on t column))
	...
	(let* ((cur-po (save-excursion (beginning-of-line) (point)))
	  ...
	  (if (or com dol)
	    ...
	    (if (setq new*match (gams-search-line))
		(progn
		  (goto-char new*match)
		  ;; Various cases.
		  (cond
		   ...
		   ((looking-at (concat "^" gams-regexp-loop))
		    ...
		    (let ((flag (if (gams-parenthesis-close-p) t nil)))
		      ...
		      (setq indent-num (gams-calculate-indent-loop new*match column flag))))
		   ;; Put block.
		   ...
		   ((looking-at gams-regexp-equation)
			...
			(setq indent-num 0)
		      ;; does not end.
		      (goto-char cur-po)
		      (setq indent-num (gams-calculate-indent-equation
					new*match new))))
...
(defun gams-comment-kill-line ()
  ...
  (let ((end (line-end-position))
    ...
    (save-excursion
      (beginning-of-line)
      (cond
       ((search-forward gams-eolcom-symbol end t)
	(delete-region (match-beginning 0) end))
       ((search-forward gams-inlinecom-symbol-start end t)
	(setq beg (match-beginning 0))
	(search-forward gams-inlinecom-symbol-end end t)
	(setq end (match-end 0))
...
(defvar gams-display-small-logo nil)
